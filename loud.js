...
d3.csv('../data/loud_analisis.csv', d3.autoType).then(data => {
  ...
  const cantRuidosas = new Map();
  data.forEach(row => {
    const category = row.category;
    if (cantRuidosas.has(category)) {
      cantRuidosas.set(category, cantRuidosas.get(category) + 1);
    } else {
      cantRuidosas.set(category, 1);
    }
  });
  
  
  
    console.log(cantRuidosas);

  compareRadialChart(cantRuidosas, 'chart3');
});
